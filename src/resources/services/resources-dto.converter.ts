...
export class ResourcesDtoConverter {
  ...
  public toModel(dto: BaseResourceRequestDto, isChild = false): Resource {
    const model = this.resourcesFactory.create(dto.name, dto.type);
    model.ownerId = dto.ownerId;
    model.ownerType = this.toOwnerType(dto.ownerType, isChild);
    model.groupId = dto.groupId;
    model.type = dto.type;
    model.name = dto.name;
    model.amount = dto.amount;
    model.resources = dto.resources?.map((child) => this.toModel(child, true));
  ...
  private toOwnerType(value: string, isChild = false): OwnerType | null {
    if (isChild) {
      return null;
    }
